...
for task in TASKS:
    ...
    ta = task(root=os.path.join("timing", task.name))
    ta.add_representation(GraphRepresentation(framework="pyg"))
    ta.get_split_loaders(batch_size=batch_size)
    model = PygModel.from_task(ta, device=device) if ta.name != "rna_vs" else VSPygModel.from_task(ta, device=device)
    ...
    print(f"Time to train model over 3 epochs: {time_training:.3f} seconds, "
