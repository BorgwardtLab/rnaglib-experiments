...
resdir = "timing_exp"
outname = 'timing_exp/timing.csv'
recompute = False

os.makedirs(resdir, exist_ok=True)
if os.path.exists(outname) or recompute:
    rows = []
    for device in ("cpu", "cuda"):
        ...
        for task in TASKS:
            ...
            model = PygModel.from_task(ta, device=device) if ta.name != "rna_vs" \
                else VSPygModel.from_task(ta, device=device)
    ...
    df.to_csv('timing_exp/timing.csv')

df = pd.read_csv('timing_exp/timing.csv', index_col=0)
