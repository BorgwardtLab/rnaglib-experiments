...
for ta_name in TASKLIST:
    for representation in REPRESENTATIONS:
        for seed in SEEDS:
            json_name = f"""results/{ta_name}_{representation}_{str(MODEL_ARGS[ta_name][representation]["num_layers"])}layers_lr{str(TRAINER_ARGS[ta_name][representation]["learning_rate"])}_{str(TRAINER_ARGS[ta_name][representation]["epochs"])}epochs_hiddendim{str(MODEL_ARGS[ta_name][representation]["hidden_channels"])}_batch_size{str(TRAINER_ARGS[ta_name][representation]["batch_size"])}_seed{str(seed)}_results.json"""
...
df.to_csv("representations_ablation.csv")
