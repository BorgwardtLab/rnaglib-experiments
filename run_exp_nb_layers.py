...
from constants import BEST_HPARAMS, TASKLIST, SEEDS
...
def run_experiment(task, split, seeds = SEEDS, shuffle = False, hparams_dict = None, rna_fm = False, representation = "2.5D", output = "wandb", project_name = "final_benchmark", retrain=False, debug = False):
    ...
    # if the hyperparameters (other than the nb of layers) are the default ones, then give a simple name to the run
    if filtered_hparams == filtered_best:
        num_layers = hparams_dict["num_layers"]
        # if the nb of layers is the default one, then call the run best_params (for consistency with other experiments)
        if num_layers == BEST_HPARAMS[task][representation][split]["num_layers"]:
            exp_name = f"""{task}_{split}_{representation}{"_rna_fm" if rna_fm else ""}_best_params"""
        else:
            exp_name = f"""{task}_{split}_{representation}{"_rna_fm" if rna_fm else ""}_{num_layers}layers"""
...
if __name__ == "__main__":
    ...
    retrain = False
    all_params = []
    for task in ["rna_cm","rna_site","rna_prot"]:
        for nb_layers in [2,3,4,5,6]:
            hparams_set = copy.deepcopy(hparams_dict[task][representation][split])
            hparams_set['num_layers'] = nb_layers
